...
public class CarRunner {
	...
	public static void testCarFilter() {
		int operationTime = 5; 
		...
		ArrayList<Car> carList = CarFactory.generateCars(20);
		var filter = new CarFilter(carList);
		
		System.out.println("Model " +">"+ make +"<" + " cars:" +"\n");
		CarDisplayer.displayCarsInfo(filter.makeFilter(make));
		
		System.out.println("Models " +">"+ model +"<"+" cars with operation time more than " +">"
				+ operationTime +"<"+ " years:" +"\n");
		CarDisplayer.displayCarsInfo(filter.ageModelFilter(model, operationTime));
		
		System.out.println("Cars with release year "+">" + releaseYear +
				"<"+ " and price higher than "+">$"+ minPrice +"<:" +"\n");
